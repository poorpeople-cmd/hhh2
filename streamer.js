...
// ==========================================
// 1️⃣ LINK EXTRACTION (PUPPETEER) - CPU OPTIMIZED
// ==========================================
async function getStreamData(isBackgroundFetch = false) {
    let modeText = isBackgroundFetch ? "BACKGROUND SWAP MODE (LIGHTWEIGHT)" : "FIRST BOOT MODE";
    console.log(`\n${"-".repeat(60)}`);
    ...
    let browserArgs = [
        ...
        '--disable-dev-shm-usage', // ⚡ GITHUB ACTIONS KE LIYE ZAROORI (RAM fix)
        '--disable-accelerated-2d-canvas', // ⚡ CPU bachane ke liye
        '--disable-gpu' // ⚡ CPU bachane ke liye
    ];
    ...
    // 🛑 CRITICAL FIX: Screen Recording sirf First Boot par hogi, Background mein NAHI!
    let recorder = null;
    if (!isBackgroundFetch) {
        recorder = new PuppeteerScreenRecorder(page);
        const videoFileName = `stream-video-cycle${fetchCycle}-${Date.now()}.mp4`;
        const videoPath = path.join(videoDir, videoFileName);
        console.log(`  🎥 [CPU OPTIMIZED] First Boot: Video recording start kar raha hoon...`);
        await recorder.start(videoPath);
    } else {
        console.log(`  ⚡ [CPU OPTIMIZED] Background Fetch: Video recording DISABLED taake FFmpeg hang na ho.`);
    }
    ...
    // 🛑 Stop recorder ONLY if it was started
    if (!isBackgroundFetch && recorder) {
